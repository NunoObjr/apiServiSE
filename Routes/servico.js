...
router.delete('/delete/:id', async (req, res) => {
    ...
    if (!ID) return res.status(400).send("Dados insuficientes")
    try {
        const serv = await Servico.findById(ID)
        if(serv.status === "Agendado"){
            return res.status(400).send({message:"Não é possivel excluir um serviço agendado, primeiro cancele o agendamento"})
        }
        const prest = await Prestador.findById(serv.prestador)
        var novoVetor = []
        for (let i = 0; i < prest.servicos.length; i++) {
            if (!serv._id.equals(prest.servicos[i])){
                novoVetor.push(prest.servicos[i])
            }
        }
        serv.deleteOne()
